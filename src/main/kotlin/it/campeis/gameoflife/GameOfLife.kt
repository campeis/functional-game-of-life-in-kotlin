...
fun <T> makeGame(
    worldContains: World<T>,
    neightbours: Neighbours<T>,
    rules: Rules,
): GameOfLife<T> =
    { alive ->
        alive
...
fun rectangularWorld(
    x: Int,
    y: Int,
): World<Cell> = { cell: Cell -> 0 <= cell.x && cell.x < x && 0 <= cell.y && cell.y < y }
...
fun circularWorld(
    ...
    { cell: Cell ->
        sqrt((cell.x - x).toDouble().pow(2) + (cell.y - y).toDouble().pow(2)) <
            radius
    }

fun unboundedWorld(): World<Cell> = { true }

private val moves2D: List<Pair<Int, Int>> =
    (-1..1).flatMap { x -> (-1..1).map { y -> Pair(x, y) } }.filter { p ->
        p.first != 0 ||
            p.second != 0
    }
